...
# php7-pecl-redis is removed as it can't be found neither
...
url_Nextcloud="https://download.nextcloud.com/server/releases/nextcloud-15.0.5.zip"
...
# 数据库安装
#mysql_install_db --user=$username --basedir=/opt --datadir=/opt/var/mysql/
mysql_install_db --user=$username --basedir=
echo -e "\n正在初始化数据库，请稍等1分钟"
sleep 20
...
sed -e "s/.*\/opt\/wwwroot\/www\/.*/    root \/opt\/wwwroot\/$2\/\;/g" -i /etc/nginx/vhost/$2.conf
}
